...
function getXYListByRect2(_min_lat, _min_lon, _max_lat, _max_lon, _level , _buffer){
...
//	var h_deg = Math.tan(Math.PI * (60 / 180));
	...
	var mrg_list = margeList(s_list.concat(w_list, n_list, e_list), _level);
...
function getXSteps(_minlon, _maxlon, _min, _max){
		...
		var steps=0;
	//		alert(_minlon + " " +_maxlon +  " " +_min.lon +  " " +_max.lon + " /"+ Math.abs(_min.lon - _max.lon));
		
		if(_minlon<0&&_maxlon>0&&_min.lon== -180&&_max.lon==-180){
			return base_steps;
...
function margeList(_arr, _level){
	var text ="";
	var newArr=[];
